...
%----------- slide --------------------------------------------------%
\begin{frame}[fragile]
\frametitle{A note on documentation}

Rather than provide urls to our documentation, I'll use:
\begin{verbatim}
DOCS:clusters
\end{verbatim}

Which means: at docs.ycrc.yale.edu, search for ``clusters''
...
%----------- slide --------------------------------------------------%
\begin{frame}[fragile]
\frametitle{Summary of Yale Clusters (Mar 2019)}
...
\regfont
\vskip 10pt
Details on each cluster at DOCS:clusters
\end{frame}
...
\vskip10pt
For more information on mobaxterm, see DOCS: mobaxterm
\end{frame}
...
This can be tricky the first time.  See DOCS: ssh
...
For more info, DOCS: Monitor CPU
...
\end{verbatim}
\item For Windows, you can use Mobaxterm for ssh, GUIs, file transfer
\item Cyberduck is a good graphical tool for Mac or Windows.  \url{https://cyberduck.io}
\item Graphical copy tools often need configuration when used with Duo (Ruddle).  
\item DOC: transfer
\end{itemize}
...
What if software you need is not currently installed as a module?

\begin{itemize}
\item General interest, ask us to install it (hpc@yale.edu)
\item R and Python packages, we've moved to DYI
\begin{itemize}
\item Python: miniconda
\item R packages: install.packages
\end{itemize}
\item bleeding-edge C/C++ apps: try DYI 
\item More info on our website
\item We'll help if you run into problems
\end{itemize}
\end{frame}
...
\end{itemize}
\item Coming soon to Farnam: Open On Demand, web-based desktop
\end{itemize}

More information, DOC: X11, VNC
...
%----------- slide --------------------------------------------------%
\begin{frame}[fragile]
\frametitle{ Issues with CPU architectures}

\begin{itemize}
\item Our clusters are heterogeneous, with multiple cpu types
\item Code compiled on new nodes (with avx2, avx512) often fails on older nodes (avx)
\item Typical error: ``Instruction Error''
\item Solutions:
\begin{enumerate}
\item Always compile on the oldest node type (we do this).
\item Always run on the newest node type
\end{enumerate}
\item Consult page for your cluster and use -C to choose node type.
\item DOC: illegal instruction
\end{itemize}
\end{frame}
...
\item Automatically schedules jobs onto a single Slurm allocation
\item Limits (walltime, memory, cpus, etc) are \em{per individual job}
\end{itemize}
...
For more info, DOC: gpus and cuda
...
\item Should you use local /tmp or /dev/shm?
\end{itemize}
\item Memory
...
\item Keep an eye on your storage quotas!
\end{itemize}
\end{itemize}
...
\item Read documentation at: \url{http://docs.ycrc.yale.edu}

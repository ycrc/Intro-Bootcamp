...
\end{itemize}
\item{Duo (ruddle,farnam) can be difficult with graphical tools}
\end{itemize}
...
Slurm: sbatch -n 16 --mem-per-cpu=20G t.sh
...
%----------- slide --------------------------------------------------%
\begin{frame}[fragile]
\frametitle{Large datasets (e.g. Genome references}
...
Qsub/Bsub/Sbatch can allocate multiple nodes, but the script runs on one core on one node sequentially.  How do we use multiple cpus?
